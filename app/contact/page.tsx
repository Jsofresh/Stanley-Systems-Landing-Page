    ...
    <>
      ...
      <main className="min-h-screen bg-[#FBFCF7] text-[#071D3A]">
        ...
        <section className={`${shell} pt-28 pb-16 lg:pt-32`}>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{["Calculator question", "AI Office Map", "Installation Sprint", "AI Office Ops", "General fit question"].map((item) => <Link key={item} href={`mailto:hello@stanley-systems.com?subject=${encodeURIComponent(item)}`} className="rounded-[1.5rem] border border-[#DDEBE2] bg-white p-6 text-xl font-semibold tracking-[-0.03em] shadow-[0_14px_38px_rgba(7,29,58,0.05)] transition hover:-translate-y-0.5 hover:border-[#15803D]">{item}</Link>)}</div>

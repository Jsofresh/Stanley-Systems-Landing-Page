...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      "That is exactly what the AI Profit Map is for. Stanley Systems checks the path from lead to job, job to invoice, invoice to payment, and customer to repeat work. You leave knowing which workflow matters first.",
  },
  {
    question: "I have been burned by consultants before. Why is this different?",
    answer:
      "Stanley Systems is not selling a giant strategy deck. The AI Profit Map finds specific admin drag points, then the build focuses on practical fixes your team can actually use: cleaner handoffs, faster billing, better follow-up, and fewer missed customers.",
  },
]
...
export function InvoicingDelayCalculatorClient() {
  ...
  const result = useMemo(() => {
    ...
    let recommendedFirstMove = "Buy the AI Profit Map to decide which AI workflow gets mapped first."
    if (!bothMeaningful && cashflowImpact > 0 && customerRevenueImpact === 0) recommendedFirstMove = "Map this in the AI Profit Map, then install it in the Sprint."
    else if (!bothMeaningful && customerRevenueImpact > 0 && cashflowImpact === 0) recommendedFirstMove = "Map this in the AI Profit Map, then install the follow-up workflow in the Sprint."
    else if (!bothMeaningful && cashflowShare >= 0.6) recommendedFirstMove = "Map this in the AI Profit Map, then install it in the Sprint."
    else if (!bothMeaningful && customerRevenueShare >= 0.6) recommendedFirstMove = "Map this in the AI Profit Map, then install the follow-up workflow in the Sprint."
  ...
  const quizContent = (() => {
    ...
    if (step === "results") {
      ...
      const ctaLabel = "Buy the AI Profit Map"
    ...
    if (step === "resultMath") {
      const summary = resultSummary
      const ctaLabel = "See how the AI Profit Map works"
            ...
            <div className="box-border w-full overflow-hidden rounded-[1.65rem] border border-[#e8dfd0] bg-white p-4 shadow-[0_22px_80px_rgba(15,23,42,0.10)] sm:rounded-[2.25rem] sm:p-6 lg:rounded-[2.5rem] lg:p-7 print:overflow-visible print:rounded-none print:border-0 print:p-0 print:shadow-none">
              ...
              <div className="hidden bg-white p-8 text-[#071D3A] print:block">
                <div className="border-b border-[#DDEBE2] pb-5">
                  ...
                  <p className="mt-2 max-w-[680px] text-sm font-medium leading-6 text-slate-600">This is a rough estimate from the calculator. The AI Profit Map checks the real records and turns the estimate into a fix list, staff AI plays, tool guidance, and the first workflow worth installing.</p>
                </div>

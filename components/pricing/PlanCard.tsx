...
export function PlanCard({ plan, featured = false }: { plan: PricingPlan; featured?: boolean }) {
  ...
  return (
    <article
      ...
      className={`flex h-full flex-col rounded-[1.75rem] border bg-white p-5 shadow-[0_18px_48px_rgba(15,23,42,0.07)] sm:p-6 ${
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          {isAudit ? (
            <h2 className="text-2xl font-semibold tracking-[-0.02em] text-[#102033]">{plan.title}</h2>
          ) : (
            <h3 className="text-2xl font-semibold tracking-[-0.02em] text-[#102033]">{plan.title}</h3>
          )}
      ...
      <p className="mt-4 text-lg font-semibold leading-7 text-[#102033]">{plan.promise}</p>
      <p className="mt-2 text-sm leading-6 text-[#536173]">{plan.description}</p>

      <div className="mt-5 rounded-2xl border border-[#e0e8ef] bg-[#f8fbfc] p-4">
        <p className="text-[2rem] font-semibold leading-none tracking-[-0.04em] text-[#102033]">{plan.priceDisplay}</p>
        <div className="mt-4 grid gap-2">
          {plan.priceRows.map((row) => (
            <div key={`${row.label}-${row.value}`} className="flex items-start justify-between gap-4 border-t border-[#e3e9ef] pt-2 text-sm leading-5">
      ...
      <ul className="mt-5 space-y-2.5">
        {plan.checklist.slice(0, 3).map((item) => (
          <li key={item} className="flex gap-2.5 text-sm font-semibold leading-6 text-[#26374b]">
      ...
      {plan.scopeNote ? (
        <p className="mt-auto pt-5 text-sm font-semibold leading-6 text-[#5f6e7d]">{plan.scopeNote}</p>
      ) : null}

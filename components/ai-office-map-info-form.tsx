...
export function AiOfficeMapInfoForm() {
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          problem: formData.question,
          main_issue: formData.question,
  ...
  return (
    <section id="map-info" className="bg-[#FBF8F2] px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
      <div className="mx-auto grid max-w-6xl gap-8 rounded-[2rem] border border-[#DDEBE2] bg-white p-6 shadow-[0_24px_70px_rgba(7,29,58,0.08)] sm:p-8 lg:grid-cols-[0.92fr_1.08fr] lg:p-10">
        <div>
          ...
          <div className="mt-6 rounded-[1.5rem] border border-[#CFE8D5] bg-[#F4FBF5] p-5 text-sm font-bold leading-6 text-[#116832]">
            Good fit: billing drag, estimate follow-up, inbox cleanup, job-note summaries, staff handoffs, or records that keep needing manual cleanup.
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid gap-4">
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-bold text-[#334B60]">What do you want to know?*</span>
            <textarea
              rows={4}
              required
              value={formData.question}
              onChange={(event) => updateField("question", event.target.value)}
              placeholder="Example: I want to know if this can help with billing prep, estimate follow-up, job notes, or admin handoffs."
              className="w-full rounded-2xl border border-[#DDEBE2] bg-[#FBFCF7] px-4 py-3.5 text-[#071D3A] outline-none transition focus:border-[#15803D] focus:ring-4 focus:ring-[#15803D]/10"
            />
          </label>
          ...
          <button type="submit" disabled={submitState === "submitting"} className="group inline-flex min-h-14 items-center justify-center gap-3 rounded-2xl bg-[#15803D] px-6 py-4 text-base font-extrabold text-white shadow-[0_16px_34px_rgba(21,128,61,0.22)] transition hover:-translate-y-0.5 hover:bg-[#116832] disabled:cursor-not-allowed disabled:opacity-70">
            {submitState === "submitting" ? "Sending..." : "Send info request"}

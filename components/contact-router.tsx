...
const routes = [
  { id: "audit", title: "Buy the AI Profit Map", body: "Best if invoices, estimates, missed calls, reviews, referrals, past customers, or cash are getting stuck in the office.", cta: "Buy the AI Profit Map", icon: ReceiptText },
  ...
  { id: "bought", title: "Already bought?", body: "Send the intake details Stanley Systems needs before reviewing the admin drag.", cta: "Continue AI Profit Map setup", icon: CheckCircle2 },
] as const
    ...
    <section className="px-4 pb-16 pt-24 sm:px-6 sm:pt-24 lg:px-8 lg:pt-28">
      <div className="mx-auto max-w-6xl">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 text-lg leading-8 text-[#536173]">Buy the AI Profit Map, see how it works, ask us a question, or continue AI Profit Map setup.</p>
        </div>
...
function AuditPanel() {
  return (
    <div>
      ...
      <h2 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-[#071D3A] sm:text-4xl">Buy the AI Profit Map.</h2>
      <p className="mt-4 text-base font-semibold leading-7 text-[#536173]">Stanley Systems finds what is slowing down cash, follow-up, reviews, referrals, and repeat work. The AI Profit Map shows what should be fixed first. If the Sprint is the right next step, Stanley Systems builds the first full version of the system. AI Office Ops is available after the build if you want Stanley Systems to keep it checked and adjusted.</p>
      <ul className="mt-5 space-y-3 text-sm font-semibold leading-6 text-[#536173]">
        {[
          ...
          "$194 credited toward your AI Office Installation Sprint.",
          "Checkout starts the AI Profit Map path. Intake gathers the work details before the review.",
        ].map((item) => <li key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[#15803D]" />{item}</li>)}
      </ul>
      <div className="mt-7 flex flex-col gap-3 sm:flex-row">
        <CTALink href={auditHref} kind="checkout" location="contact_audit_panel" analyticsEvent="audit_checkout_clicked" analyticsSource="contact_router" packageId="workflow_audit" packageName="AI Profit Map" billingPeriod="one_time" ctaLabel="Buy the AI Profit Map" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-13 items-center justify-center rounded-full bg-[#15803D] px-6 py-3 text-sm font-extrabold text-white transition hover:-translate-y-0.5 hover:bg-[#116832] hover:shadow-[0_18px_36px_rgba(21,128,61,0.22)]">
          Buy the AI Profit Map <ArrowRight className="ml-2 h-4 w-4" />
        </CTALink>
...
function BoughtPanel() {
  return (
    <div>
      <h2 className="text-3xl font-semibold tracking-[-0.03em] text-[#071D3A] sm:text-4xl">Start your AI Profit Map intake.</h2>
      <p className="mt-4 text-base font-semibold leading-7 text-[#536173]">If you already bought the AI Profit Map, use the intake route to send the work details Stanley Systems needs before reviewing how money and follow-up move now.</p>
      <Link href="/audit-intake" className="mt-7 inline-flex min-h-13 items-center justify-center rounded-full bg-[#15803D] px-6 py-3 text-sm font-extrabold text-white transition hover:-translate-y-0.5 hover:bg-[#116832] hover:shadow-[0_18px_36px_rgba(21,128,61,0.22)]">Go to AI Profit Map intake <ArrowRight className="ml-2 h-4 w-4" /></Link>
    </div>
  )
}
...
function PreBuyQuestionForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("success")
      setMessage(result?.message || "Thanks. Stanley Systems received your question. If it belongs in the AI Profit Map, we’ll point you there instead of guessing.")
  ...
  return (
    <form onSubmit={submit} className="grid gap-4">
      <div>
        ...
        <p className="mt-3 text-sm font-semibold leading-6 text-[#536173]">Not sure if this fits? Ask us a question before you start. We’ll help you decide whether the AI Profit Map is the right next step.</p><p className="mt-2 text-sm font-semibold leading-6 text-[#607080]">You may first speak with our phone assistant so Stanley Systems can route the question quickly.</p><p className="mt-2 text-sm font-semibold leading-6 text-[#607080]">Short questions are fine. If you need diagnosis, start the AI Profit Map so Stanley Systems can check the real business process.</p>
      </div>

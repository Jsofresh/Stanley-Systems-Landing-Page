    ...
    <section id="packages" data-section="fit-access-pricing" data-nav-theme="light" className="bg-white px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
      <div className={page.wrap}>
        <div className="max-w-4xl">
          <h2 className={`${page.h2} mt-3`}>Get the AI Office Map. Find the office work your business should not be paying for anymore.</h2>
        ...
        <div data-section="cash-flow-assessment-access-offer" className="mt-5 grid gap-5 lg:grid-cols-[0.66fr_1.2fr] lg:items-center">
          ...
          <aside className="rounded-[1.75rem] border border-[#cfe8d5] bg-white p-5 text-[#071D3A] shadow-[0_16px_46px_rgba(7,29,58,0.08)]">
            ...
            <div className="mt-3 flex items-end gap-2"><span className="text-5xl font-semibold tracking-[-0.04em] text-[#071D3A]">$197</span><span className="pb-2 text-sm font-bold text-[#536173]">assessment</span></div>
            ...
            <div className="mt-5 rounded-2xl border border-[#efb7b0] bg-[#fff6f4] p-4 text-[#b91c1c]">
              ...
              <p className="mt-1 text-2xl font-black tracking-[-0.035em]">-$197 monthly / -$194 yearly</p>
            </div>
            <CTALink href={assessmentHref} kind="checkout" location="workflow_audit_pricing" analyticsEvent="audit_checkout_clicked" analyticsSource="workflow_audit_page" packageId="workflow_audit" packageName="AI Office Map" billingPeriod="one_time" ctaLabel="Get the AI Office Map" target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[#15803D] px-6 py-3 text-sm font-extrabold text-white transition hover:bg-[#116832] focus:outline-none focus:ring-4 focus:ring-[#bbf7d0]">Get the AI Office Map <ArrowRight className="ml-2 h-4 w-4" /></CTALink>
          </aside>
        </div>
        ...
        <p className="mt-5 text-center text-sm font-semibold leading-6 text-[#536173]">Prefer to buy a system directly? You can buy AI Office Installation Sprint, AI Office Ops, or AI Office Installation Sprint + Ops when the leak is already clear. Get the AI Office Map if you are not sure which system should go first.</p>
      </div>
    </section>
  )
}

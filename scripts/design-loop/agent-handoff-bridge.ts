...
Human-directed section-build requirements:
- The active workflow is Jaden-approved ChatGPT/mockup -> Hermes task packaging -> Codex visual inspection and pre-build interpretation -> Codex build -> Hermes screenshots/verification -> Jaden screenshot approval -> commit/deploy only after explicit approval.
- Do not build from Hermes text summaries alone when an approved mockup image is provided. Inspect the actual approved mockup image bytes before editing. Inspect the current Playwright screenshot directly when provided.
- If the task names an approved mockup image path, Jaden approval note, locked section copy, section job, allowed files, protected files, current screenshot, bad-pattern reminders, Stanley website design workflow standard, Stanley Refero style synthesis, section-specific Refero/DESIGN.md research pack, approved taste examples, rejected bad patterns, or pre-build interpretation path, treat them as required inputs.
- Before editing code, write the Codex pre-build interpretation artifact at the task-specified path. It must describe what Codex sees in the approved mockup, current screenshot critique if provided, section job, locked copy/CTA hierarchy, component tree, visual hierarchy, spacing/container rules, mobile plan, real DOM text requirements, what not to copy literally, bad patterns, and acceptance criteria. If Refero/DESIGN.md research is provided, cite the concrete principles used and avoided.
- If you cannot visually inspect the mockup/screenshot pixels accurately, stop, mark the task blocked, and report that Codex cannot act as visual builder for that task yet.
- Do not paste full-section mockup PNGs into the website; rebuild as real React/Tailwind/SVG/DOM. Do not use the approved mockup as production UI.
- Do not approve your own work. A local pass means ready for Hermes verification/Jaden screenshot checkpoint, not final taste approval or deploy approval.

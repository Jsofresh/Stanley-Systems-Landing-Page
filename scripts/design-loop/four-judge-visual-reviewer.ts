 ...
 *
 * Naming note: this legacy runner actually executes four model roles.
 * The active critical visual review gate uses the Smart Vision/Judge 5 command path.
 *
 * Usage:
...
const STANLEY_DESIGN_PATH = process.env.STANLEY_DESIGN_PATH || "/home/jaden/stanley-landing/DESIGN.md"
...
function loadStanleyContext(): StanleyContext {
  ...
  const chunks: string[] = [
    "# Current Stanley Systems Review Context",
    "",
    "DESIGN.md controls visual style. The live website is visual comparison only and never content authority. Current shared-context files control offer, copy, and product truth.",
  ]

  if (!existsSync(STANLEY_DESIGN_PATH)) {
    missing.push(STANLEY_DESIGN_PATH)
  } else {
    loaded.push(STANLEY_DESIGN_PATH)
    chunks.push(`\n---\n\n## Canonical visual source: ${STANLEY_DESIGN_PATH}\n\n${readFileSync(STANLEY_DESIGN_PATH, "utf8").trim()}\n`)
  }
  ...
  if (!loaded.includes(STANLEY_DESIGN_PATH)) {
    throw new Error(`Canonical Stanley Systems visual source did not load: ${STANLEY_DESIGN_PATH}`)
  }
  if (loaded.length < 2) {
    throw new Error(`No current Stanley Systems business context files loaded from ${STANLEY_CONTEXT_ROOT}`)
  }
...
function buildStrategyPrompt(packet: ReviewPacket, route: string, visibleText: string, context: StanleyContext): string {
  return [
    ...
    context.markdown,
    "<<<STANLEY_CONTEXT_END>>>",
    "Strategy rubric: judge ICP alignment, current approved offer/copy architecture from the supplied shared context, plain public language, result-first positioning, and truthful representation of the installed AI office workflow.",
...
function buildFinalGatekeeperPrompt(packet: ReviewPacket, route: string, visibleText: string, context: StanleyContext, freshRaw: VisionResult, mobileRaw: VisionResult, strategyRaw: VisionResult, fresh: JudgeDecision, mobile: JudgeDecision, strategy: JudgeDecision): string {
  ...
  return [
    ...
    "- Source boundary: the live Stanley Systems website is visual evidence for style, color, formatting, imagery, and interaction only. Its current content, offer names, prices, claims, CTA labels, routes, and funnel order are not authority.",
...
function validateFinalDecision(value: unknown, fresh: JudgeDecision, mobile: JudgeDecision, strategy: JudgeDecision, context: StanleyContext): FinalDecision {
  ...
  if (decision.repeated_card_pattern_dominates) positiveVisualFailures.push("repeated_card_pattern_dominates is true")
  if (decision.repeated_card_pattern_present && !decision.repeated_card_pattern_is_secondary_support) positiveVisualFailures.push("repeated_card_pattern_present is true but not marked as secondary support")
  if (decision.repeated_card_pattern_present && decision.repeated_card_pattern_is_secondary_support && (decision.visual_anchor_score < 8 || decision.imagery_strength_score < 8 || !decision.visual_anchor_overpowers_card_stack || !decision.primary_visual_anchor_description.trim())) positiveVisualFailures.push("repeated cards are present but not justified by a strong described visual anchor overpowering the card stack")

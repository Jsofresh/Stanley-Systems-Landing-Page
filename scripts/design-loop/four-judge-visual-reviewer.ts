...
function loadStanleyContext(): StanleyContext {
  ...
  const chunks: string[] = ["# Locked Stanley Systems Review Context", "", "Generated by scripts/design-loop/four-judge-visual-reviewer.ts from canonical context files. File path retained for compatibility; this legacy runner executes four model roles."]
...
function buildStrategyPrompt(packet: ReviewPacket, route: string, visibleText: string, context: StanleyContext): string {
  return [
    ...
    context.markdown,
    "<<<STANLEY_CONTEXT_END>>>",
    "Strategy rubric: judge ICP alignment, offer architecture, copy rules, public language rules, result-first positioning, Office Process Assessment, Cash Flow Collection System, and Repeat Revenue System.",

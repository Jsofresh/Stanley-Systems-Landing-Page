...
const PROJECT_ROOT = "/home/jaden/stanley-assets/openclaw-project"
...
function buildCustomerRevenuePack(args: Args) {
  ...
  const entries = [
    ...
    source(offer, "offer architecture", ["Office Process Assessment is paid diagnostic wedge", "Repeat Revenue System public promise: Get more money from the customers you already earned", "Stanley Systems brings past customers back, gets reviews, creates referrals, catches extra calls", "public copy says Stanley Systems"]),
...
if (one(args, "help") === "true") {
  console.log("Usage: node --experimental-strip-types scripts/design-loop/build-stanley-image-context-pack.ts --section_id customer-revenue --section_type 'Repeat Revenue System' --route / --run_id <id>")
} else {

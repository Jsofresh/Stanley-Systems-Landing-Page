...
export type WorkflowAdapterCommand =
  | { operation: "start_work"; sequence: number; at: string; label?: string }
  | { operation: "get_status"; sequence: number; at: string; phase?: WorkflowPhase; label: string; detail?: string }
  | { operation: "approve"; sequence: number; at: string; approval: WorkflowApproval; decision?: "Approve" | "Cancel" }
  | { operation: "cancel"; sequence: number; at: string; confirmed: boolean }
  | { operation: "get_result"; sequence: number; at: string; receipt?: WorkflowReceipt; error?: string }
...
export function applyWorkflowCommand(
  ...
  if (command.operation === "start_work") {
    ...
    return { ...state, operation: command.operation, phase, sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: undefined, receipt: undefined, notice: undefined }
  }
  ...
  if (command.operation === "get_status") {
    ...
    return { ...state, operation: command.operation, phase, sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), notice: undefined }
  }
  ...
  if (command.operation === "approve") {
    if (command.decision === "Approve") {
      ...
      return { ...state, operation: command.operation, phase: "running", sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: undefined, notice: undefined }
    }
    if (command.decision === "Cancel") {
      ...
      return { ...state, operation: "cancel", phase: "cancelling", sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: undefined, notice: undefined }
    }
    ...
    return { ...state, operation: command.operation, phase: "approval_required", sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: command.approval, notice: undefined }
  }
  ...
  if (command.operation === "cancel") {
    ...
    return { ...state, operation: command.operation, phase, sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: undefined, notice: undefined }
  }
  ...
  if (!command.receipt) {
    ...
    return { ...state, operation: command.operation, phase: "unknown_outcome", sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: undefined, receipt: undefined, notice: command.error ?? "The result payload could not be verified." }
  }
  ...
  return { ...state, operation: command.operation, phase, sequence: command.sequence, updatedAt: command.at, history: boundedHistory(state, item), approval: undefined, receipt: command.receipt, notice: undefined }
}
...
function resultStatus(completion: ReceiptCompletion): WorkflowReceipt["resultSummary"]["status"] {
  ...
  if (completion.work_result?.status === "verified" && completion.status === "completed") return "completed"
...
function reconciliationStatus(completion: ReceiptCompletion): WorkflowReceipt["reconciliationStatus"] {
  ...
  if (result.status === "verified") {
    return completion.provider_verification?.status === "verified" || completion.provider_verification?.status === "not_applicable"
      ? "reconciled"
...
function readbacks(completion: ReceiptCompletion): WorkflowProviderReadback[] {
  ...
  return verification.connectors.map((connector) => {
    ...
    const status: WorkflowProviderReadback["status"] = rejected ? "rejected" : verification.status === "verified" && verified ? "verified" : "unknown"
    return {
...
export function canSaveWorkflowAsRoutine(receipt: WorkflowReceipt | undefined, tenantId: string) {
  return Boolean(
    ...
    && receipt.providerReadback.every((readback) => readback.status === "verified" || readback.status === "not_applicable"),
  )
}
